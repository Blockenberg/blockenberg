...
<div
  class="container mx-auto grid grid-cols-1 justify-center gap-6 text-center dark:bg-stone-800 sm:grid-cols-2 md:p-6 md:py-20 lg:grid-cols-4 lg:px-8"
>
  {#each $cmsStore.selectedArea === AREAS.PRIVATE ? $cmsStore.privateDocuments : $cmsStore.publicDocuments as doc}
    <a
      ...
      class="group mx-auto w-full rounded-md border-2 border-stone-50/5 bg-stone-50 transition-all  delay-150 duration-1000 hover:border-violet-600/50 hover:bg-violet-50 dark:bg-stone-900"
    >
      {#if doc.src}
        <img
          role="presentation"
          class="h-44 w-full rounded-t-md object-cover dark:bg-stone-500"
          alt="Cover image for {decodeURI(doc.header)}"
          src={doc.src}
        />
      {/if}
      <div class="space-y-2 overflow-clip truncate p-6">
        <h3 class="text-3xl font-semibold group-focus:underline">
        ...
        {#if !doc.private}
          <a
            href="https://ipfs.runfission.com/ipfs/{doc.cid}/userland"
            class="text-xs text-stone-500 mx-4"
          >
            {doc.cid}
          </a>
        {/if}
...
<section>
  <div
    class="container mx-auto hidden p-6 py-20 text-center dark:bg-stone-900 lg:px-8"
  >
    <div>
      <a
    ...
    <p class="mt-6">
      ...
      all. A bad writing can be corrected. An empty page remains an empty page.”
    </p>
    ...
    <img
      ...
      alt=""
      class="mt-8 h-auto max-h-96 w-full object-cover object-[center_30%] dark:bg-stone-500"
    />
  </div>
</section>
