 ...
 * @param name
 */
export const getImageFromWNFS: (name: string) => Promise<Image> = async (
	name,
) => {
	try {
		const fs = getStore(filesystemStore);
 ...
 * @param name
 */
export const getDocFromWNFS: (name: string) => Promise<{
	cid: string;
	ctime: number;
	name: string;
	header: string;
 ...
 * @param image
 */
export const uploadImageToWNFS: (image: File) => Promise<{
	cid: string;
	name: string;
	size: number;
	src: string;
 ...
 * @param name
 */
export const deleteDocFromWNFS: (name: string) => Promise<void> = async (
	name,
) => {
	try {
		const { selectedArea } = getStore(cmsStore);
 ...
 */
export const handleFileInput: (files: FileList) => Promise<void> = async (
	files,
) => {
	await Promise.all(
