...
export const uploadFileToWNFS: (
  ...
  try {
    ...
    const imageExists = await fs.exists(
      wn.path.file(...GALLERY_DIRS[ selectedArea ], file.name)
    )
    if (imageExists) {
      throw new Error(`${file.name} image already exists`)
    }
    ...
    addNotification(`${file.name} image has been published`, 'success')
  } catch (error) {
